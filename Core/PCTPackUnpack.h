...
{	
	...
	DAT_SPO2_LEAD = 0x03,         //血氧导联信息
	DAT_SPO2_DATA = 0x04,         //血氧数据
...
u8    PackData(StructPCTPackType* pPT);      //对数据进行打包，1-打包成功，0-打包失败
u8    UnPackData(u8 data);                //对数据进行解包，1-解包成功，0-解包失败

StructPCTPackType GetUnPackRslt(void);      //读取解包后数据包